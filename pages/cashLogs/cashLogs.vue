				...
				<view class="day-group" v-for="(val,key) in item.list" :key="key">
					<view class="day-wrap border-bottom">
						<view class="day-title flex-row">
							<view class="">{{moment(val.addTime).format('MM月DD日 HH:mm:ss')}}</view>
							<view class="">{{stateMap[val.state]}}</view>
						</view>
						<view class="day-content flex-row">
							<view>
								<text>-￥{{(val.cashAmount || 0).toFixed(2)}}</text>
								<text class="tag" v-if="val.feeAmount">手续费:￥{{val.feeAmount}}</text>
							</view>
							<view class="remark acp-ellipsis flex-1" v-if="val.state == 9">失败原因：{{val.remark}}</view>
						</view>
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			getList() {
				cashList(this.queryList).then(res => {
